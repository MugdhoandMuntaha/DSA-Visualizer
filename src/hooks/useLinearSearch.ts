...
export interface LSItem { val: number; id: string; }

export type LSPhase = "init" | "searching" | "found" | "not-found";
...
export function useLinearSearch(initialArray: number[], initialTarget: number) {
  const [running, setRunning] = useState(false);
  const [paused, setPaused] = useState(false);
  
  const toLS = (arr: number[]) => arr.map((v, i) => ({ val: v, id: `v-${v}-${i}` }));
  const [arrayState, setArrayState] = useState<LSItem[]>(toLS(initialArray));
  const [targetState, setTargetState] = useState<number>(initialTarget);
  const [currentState, setCurrentState] = useState<LSState | null>(null);
  const [activeStepIdx, setActiveStepIdx] = useState(-1);
  const [logEntries, setLogEntries] = useState<{ type: string; msg: string }[]>([]);

  const stepsRef = useRef<LSState[]>([]);
  ...
  const buildSteps = useCallback((arr: number[], target: number) => {
    ...
    const mk = (
      type: string, phase: LSPhase, msg: string, currIdx: number | null
    ): LSState => {
      return {
        originalArr: [...a], target, currIdx, phase, msg, type
      };
    };

    steps.push(mk("init", "init", `Initialize Linear Search for target = ${target}.`, null));

    for (let i = 0; i < n; i++) {
      steps.push(mk("compare", "searching", `Check index ${i}: Compare arr[${i}] (${a[i].val}) with target (${target}).`, i));

      if (a[i].val === target) {
        steps.push(mk("found", "found", `Match! Target ${target} found at index ${i}.`, i));
        return steps;
      } else {
        steps.push(mk("move-next", "searching", `arr[${i}] !== target. Moving to next element.`, i));
      }
    }

    steps.push(mk("not-found", "not-found", `Reached end of array. Target ${target} not found.`, null));
