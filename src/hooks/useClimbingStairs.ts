...
export type CSMode = "tabulation" | "recursive" | "memoization";
...
export const useClimbingStairs = () => {
  const [csState, setState] = useState<CSState>({
    n: 5,
    mode: "recursive",
    dp: [],
    tree: {},
  ...
  const generateFrames = (n: number, mode: CSMode) => {
    framesRef.current = [];
    currentFrameRef.current = 0;
    
    let currentState: CSState = {
      n, mode,
      dp: Array(n + 1).fill(null),
      tree: {}, rootId: null, callStack: [], logs: [`Starting Climbing Stairs ${mode}`],
      activeCodeLine: null, currI: -1, highlightCells: [], backtrackingEdge: null, isFinished: false
    };

    const push = (line: number | null, msg: string, updates: Partial<CSState> = {}) => {
    ...
    if (mode === "tabulation") {
      currentState.dp[1] = 1;
      push(3, `dp[1] = 1 (1 way to reach step 1)`, { dp: [...currentState.dp] });
      
      if (n >= 2) {
        currentState.dp[2] = 2;
        push(4, `dp[2] = 2 (2 ways to reach step 2)`, { dp: [...currentState.dp] });
      }

      for (let i = 3; i <= n; i++) {
        push(5, `Calculating dp[${i}]`, { currI: i });
        currentState.dp[i] = currentState.dp[i - 1]! + currentState.dp[i - 2]!;
        push(6, `dp[${i}] = dp[${i-1}] + dp[${i-2}] = ${currentState.dp[i]}`, { 
            dp: [...currentState.dp],
            highlightCells: [{ i: i - 1, color: "rgba(34,197,94,0.3)" }, { i: i - 2, color: "rgba(34,197,94,0.3)" }]
        });
      }
      push(8, `Finished tabulation. Result: ${currentState.dp[n]}`, { currI: -1, highlightCells: [{ i: n, color: 'rgba(168,85,247,0.8)' }], isFinished: true });
      ...
      const solve = (i: number, parentId: string | null, edgeLabel: string = "", direction: "left" | "right" = "left"): number => {
        ...
        const newStack = [...currentState.callStack, { func: "climbStairs", args: { n: i } }];
        push(mode === "recursive" ? 0 : 0, `Called climbStairs(${i})`, { tree: newTree, callStack: newStack, currI: i });
        ...
        if (i <= 2) {
           ...
           push(1, `Base case hit. climbStairs(${i}) = ${i}`, { tree: { ...currentState.tree, [id]: latestNode }, backtrackingEdge: currentState.backtrackingEdge });
        ...
        push(mode === "recursive" ? 2 : 3, `Recursing on climbStairs(${i-1}) and climbStairs(${i-2})`);
        const left = solve(i-1, id, "-1 step", "left");
        const right = solve(i-2, id, "-2 steps", "right");
  ...
  const reset = (n: number, mode: CSMode) => {
    if (timerRef.current) clearInterval(timerRef.current);
    initData(n, mode);
  };

  return { csState, running, paused, speed, setSpeed, generateFrames, pause, resume, reset };
};
