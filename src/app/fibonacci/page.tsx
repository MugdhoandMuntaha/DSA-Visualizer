...
import { useFibonacci, FIB_CODES, FibMode } from '@/hooks/useFibonacci';
import { FibonacciVisualizer } from '@/components/FibonacciVisualizer';

export default function FibonacciPage() {
  const fib = useFibonacci();
  const [nInput, setNInput] = useState(5);
  const [mode, setMode] = useState<FibMode>("recursive");

  useEffect(() => {
    fib.reset(nInput, mode);
  }, []);

  const handleStart = () => {
    fib.generateFrames(nInput, mode);
  };

  const handleModeChange = (event: React.MouseEvent<HTMLElement>, newMode: FibMode | null) => {
    if (newMode !== null) {
      setMode(newMode);
      fib.reset(nInput, newMode);
    }
  };
      ...
      <Box sx={{ flex: "none", p: 2, display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.1)", bgcolor: "rgba(15,23,42,0.8)", backdropFilter: "blur(10px)" }}>
        <Box>
          <Typography variant="h5" sx={{ fontWeight: 700, color: "#fff", display: "flex", alignItems: "center", gap: 1.5 }}>
            <span style={{ color: "#e11d48" }}>Fibonacci Sequence</span>
          </Typography>
        ...
        <Box sx={{ display: "flex", gap: 3, alignItems: "center" }}>
          ...
          <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
            <TextField
              ...
              label="Value of n"
              value={nInput}
              onChange={(e) => setNInput(Math.min(15, Math.max(0, parseInt(e.target.value) || 0)))}
              disabled={fib.running}
              sx={{ width: 120, input: { color: "#fff" }, label: { color: "text.secondary" } }}
              slotProps={{ htmlInput: { min: 0, max: 15 } }}
            />
            <Button variant="contained" sx={{ bgcolor: "#e11d48", "&:hover": { bgcolor: "#be123c" } }} onClick={handleStart} disabled={fib.running || nInput < 0}>
              Start DP
            </Button>
          </Box>
        </Box>
      ...
      <Box sx={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <Box sx={{ width: 400, flex: "none", borderRight: "1px solid rgba(255,255,255,0.1)", display: "flex", flexDirection: "column", p: 2, gap: 2, bgcolor: "rgba(0,0,0,0.2)" }}>
          <Paper variant="outlined" sx={{ p: 2, borderColor: "rgba(225,29,72,0.15)", flex: "none" }}>
            <Typography variant="overline" sx={{ fontSize: "0.78rem", fontWeight: 700, color: "text.secondary", mb: 1, display: "block" }}>Fibonacci Code ({mode})</Typography>
            <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5, bgcolor: "rgba(0,0,0,0.3)", p: 1, borderRadius: 1 }}>
              {FIB_CODES[mode].map((line, idx) => (
                <Typography
                  key={idx}
                  variant="caption"
                  sx={{
                    ...
                    fontSize: "0.7rem",
                    color: fib.fibState.activeCodeLine === idx ? "#fff" : "text.secondary",
                    bgcolor: fib.fibState.activeCodeLine === idx ? "rgba(225,29,72,0.3)" : "transparent",
                    py: 0.25,
                    px: 0.5,
                    borderRadius: 0.5,
                    borderLeft: fib.fibState.activeCodeLine === idx ? "2px solid #e11d48" : "2px solid transparent",
                    transition: "all 0.2s"
                  }}
                >
          ...
          <Paper variant="outlined" sx={{ p: 2, borderColor: "rgba(225,29,72,0.15)", flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
            ...
            <Box sx={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 0.5 }}>
              {fib.fibState.logs.map((log, idx) => (
                <Typography key={idx} variant="caption" sx={{ fontFamily: "var(--font-mono)", color: idx === fib.fibState.logs.length - 1 ? "#fff" : "text.secondary" }}>
                  {log}
                </Typography>
              ))}
            </Box>
          ...
          {(mode === "memoization" || mode === "recursive") && (
            <Paper variant="outlined" sx={{ p: 2, borderColor: "rgba(225,29,72,0.15)", flex: "none", minHeight: 120 }}>
              ...
              {fib.fibState.callStack && fib.fibState.callStack.length > 0 ? (
                <Box sx={{ display: "flex", flexDirection: "column-reverse", gap: 0.5 }}>
                  {fib.fibState.callStack.map((frame, idx) => (
                    <Box key={idx} sx={{ p: 0.75, borderRadius: 1, bgcolor: idx === fib.fibState.callStack.length - 1 ? "rgba(225,29,72,0.15)" : "action.hover", border: "1px solid", borderColor: idx === fib.fibState.callStack.length - 1 ? "primary.main" : "divider", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <Typography sx={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem", fontWeight: idx === fib.fibState.callStack.length - 1 ? 700 : 500, color: idx === fib.fibState.callStack.length - 1 ? "primary.main" : "text.primary" }}>
                        {frame.func}()
                      </Typography>
          ...
          <Paper variant="outlined" sx={{ p: 2, borderColor: "rgba(225,29,72,0.15)", mt: "auto" }}>
             ...
             <Slider min={1} max={10} defaultValue={5} onChange={(_, v) => fib.setSpeed(v as number)} size="small" sx={{ mb: 1.5, color: "#e11d48" }} />
             <Box sx={{ display: "flex", gap: 1 }}>
               <Button variant="outlined" fullWidth size="small" startIcon={fib.paused ? <PlayArrow /> : <Pause />} onClick={fib.paused ? fib.resume : fib.pause} disabled={!fib.running && !fib.paused}>
                 {fib.paused ? "Resume" : "Pause"}
               </Button>
               <Button variant="outlined" fullWidth size="small" color="error" startIcon={<RestartAlt />} onClick={() => fib.reset(nInput, mode)} disabled={fib.running}>
                 Reset
               </Button>
             </Box>
          </Paper>
        ...
        <Box sx={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", p: 3 }}>
          {mode === "memoization" && fib.fibState.dp && fib.fibState.dp.length > 0 && (
          ...
          <Box sx={{ flex: 1, minHeight: 0, overflow: "auto", display: "flex", justifyContent: "center", alignItems: "center" }}>
             <FibonacciVisualizer fibState={fib.fibState} />
          </Box>
        </Box>
      </Box>
    </Box>
