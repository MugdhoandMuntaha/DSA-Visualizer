...
const RS_STEPS: StepDef[] = [
  { id: "init", title: "Initialize", description: "Find max value to determine number of digit passes needed." },
  { id: "start-pass", title: "Start Pass", description: "Initialize 10 empty buckets for the current digit place." },
  { id: "distribute", title: "Distribute", description: "Read current digit of each element and place into correct bucket." },
  { id: "placed", title: "Placed", description: "Element placed into its bucket." },
  { id: "collect-start", title: "Collect", description: "Gather elements from buckets 0→9 back into the array." },
  { id: "collect", title: "Collecting", description: "Element collected from bucket into its new position." },
  { id: "pass-done", title: "Pass Done", description: "Array is now sorted by this digit. Move to next." },
  { id: "done", title: "Done", description: "All digit passes complete. Array fully sorted." },
];

export default function RadixSortPage() {
  const [baseArray, setBaseArray] = useState([170, 45, 75, 90, 802, 24, 2, 66]);
  const rs = useRadixSort(baseArray);
  ...
  const isDone = rs.logEntries.some(l => l.type === "done");

  const handleUpdate = () => {
    if (rs.running) return;
    const arr = arrayInput.split(",").map(x => parseInt(x.trim())).filter(x => !isNaN(x) && x >= 0);
    if (arr.length > 0 && arr.length <= 14 && arr.every(v => v <= 999)) {
      setBaseArray(arr);
      setResultDialogDismissed(false);
      rs.resetSort();
    } else {
      alert("Enter 1–14 non-negative numbers (max 999) for a clean visualization.");
    }
  };
  ...
  const handlePreset = (t: "random" | "reversed" | "classic") => {
    if (rs.running) return;
    let a: number[];
    if (t === "random") a = Array.from({ length: 8 }, () => Math.floor(Math.random() * 900) + 10);
    else if (t === "reversed") a = [900, 750, 600, 450, 300, 150, 75, 10];
    else a = [170, 45, 75, 90, 802, 24, 2, 66]; // textbook example
    setBaseArray(a);
  ...
  const handleStart = () => {
    setResultDialogDismissed(false);
    rs.startSort(baseArray);
  };

  const tagColor: Record<string, string> = {
    init: "#7c3aed", "start-pass": "#6366f1", distribute: "#eab308", placed: "#f97316",
    "collect-start": "#06b6d4", collect: "#3b82f6", "pass-done": "#10b981", done: "#14b8a6",
  };

  return (
    <Box sx={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <Header title="Radix Sort Visualizer" badge="O(N·K) · Non-Comparative · LSD" />
      <Box sx={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <Box sx={{ width: 300, minWidth: 300, display: "flex", flexDirection: "column", gap: 1.5, p: 1.5, overflowY: "auto",
          ...
          <Paper variant="outlined" sx={{ p: 2, borderColor: "rgba(99,102,241,0.15)" }}>
            ...
            <TextField fullWidth size="small" value={arrayInput} onChange={e => setArrayInput(e.target.value)}
              disabled={rs.running} placeholder="e.g. 170, 45, 75, 90"
              helperText="Up to 14 non-negative integers ≤ 999"
              sx={{ mb: 1.5, "& .MuiInputBase-input": { fontFamily: "var(--font-mono)", fontSize: "0.8rem" } }} />
            <Button fullWidth variant="outlined" size="small" onClick={handleUpdate} disabled={rs.running}
            ...
            <Box sx={{ display: "flex", gap: 0.5, alignItems: "center" }}>
              ...
              <Tooltip title="Classic Example"><IconButton size="small" onClick={() => handlePreset("classic")} disabled={rs.running}>
                <Typography sx={{ fontWeight: 900, fontSize: "0.7rem" }}>EG</Typography>
              </IconButton></Tooltip>
              <Tooltip title="Random"><IconButton size="small" onClick={() => handlePreset("random")} disabled={rs.running}><Shuffle sx={{ fontSize: 16 }} /></IconButton></Tooltip>
              <Tooltip title="Reversed"><IconButton size="small" onClick={() => handlePreset("reversed")} disabled={rs.running}><SignalCellularAlt sx={{ fontSize: 16, transform: "scaleX(-1)" }} /></IconButton></Tooltip>
            </Box>
          </Paper>
          ...
          <Paper variant="outlined" sx={{ p: 2, borderColor: "rgba(99,102,241,0.15)" }}>
            ...
            <Slider min={1} max={10} defaultValue={5} onChange={(_, v) => rs.setSpeed(v as number)} size="small" sx={{ mb: 1.5, color: "primary.main" }} />
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.75 }}>
              <Button variant="contained" size="small" startIcon={<PlayArrow />} onClick={handleStart}
                disabled={baseArray.length === 0 || (rs.running && !rs.paused)}
              ...
              <Button variant="outlined" size="small" startIcon={<SkipNext />} onClick={() => rs.stepSort(baseArray)}
                sx={{ flex: "1 1 calc(50% - 4px)", fontSize: "0.75rem" }}>Step</Button>
              <Button variant="outlined" size="small" startIcon={rs.paused ? <PlayArrow /> : <Pause />} onClick={rs.togglePause} disabled={!rs.running}
                sx={{ flex: "1 1 calc(50% - 4px)", fontSize: "0.75rem" }}>{rs.paused ? "Resume" : "Pause"}</Button>
              <Button variant="outlined" size="small" color="error" startIcon={<RestartAlt />} onClick={rs.resetSort}
          ...
          <Paper variant="outlined" sx={{ p: 2, borderColor: "rgba(99,102,241,0.15)" }}>
            ...
            <AlgoSteps steps={RS_STEPS} activeIdx={rs.activeStepIdx} isDone={isDone} />
          </Paper>
        </Box>

        {/* ─── Visualization ─── */}
        <RadixSortVisualizer initialArray={baseArray} currentState={rs.currentState} />
        ...
        <Box sx={{ width: 340, minWidth: 340, display: "flex", flexDirection: "column", gap: 1.5, p: 1.5, overflowY: "auto",
          ...
          {rs.currentState && rs.currentState.phase !== "init" && (
            <Paper variant="outlined" sx={{ p: 2, borderColor: "rgba(99,102,241,0.15)" }}>
              ...
              <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                {[
                  { label: "Digit Place", val: rs.currentState.digitLabel, color: "#eab308" },
                  { label: "Pass", val: `${rs.currentState.pass} / ${rs.currentState.totalPasses}`, color: "#6366f1" },
                  { label: "Active Bucket", val: rs.currentState.activeBucket !== null ? rs.currentState.activeBucket : "-", color: "#f97316" },
                ].map((item, idx) => (
      ...
      <Dialog open={isDone && !resultDialogDismissed} onClose={() => setResultDialogDismissed(true)}
        ...
        <DialogContent>
          <DialogContentText sx={{ color: "text.secondary", fontSize: "1.1rem" }}>
            Array fully sorted using Radix Sort (LSD) in {rs.currentState?.totalPasses ?? "?"} digit pass{(rs.currentState?.totalPasses ?? 1) > 1 ? "es" : ""}.
          </DialogContentText>
        </DialogContent>
