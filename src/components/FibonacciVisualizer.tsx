...
import { FibState, TreeNode } from '../hooks/useFibonacci';
...
const TreeEdges: React.FC<{ tree: Record<string, TreeNode>, backtrackingEdge: any }> = ({ tree, backtrackingEdge }) => {
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      const newEdges: any[] = [];
      const containerEl = document.getElementById("fib-tree-container");
...
export const FibonacciVisualizer: React.FC<{ fibState: FibState }> = ({ fibState }) => {
  const { mode, n, dp, tree, rootId, currI, highlightCells, backtrackingEdge, isFinished } = fibState;
  ...
  return (
    <Box id="fib-scroll-container" sx={{ width: "100%", height: "100%", overflow: "auto", display: "flex", flexDirection: "column", pt: 4, px: 4, position: "relative" }}>
          ...
          {isFinished && (
            <Box sx={{ mt: 2, p: 2, bgcolor: "rgba(168,85,247,0.1)", borderRadius: 1, border: "1px solid", borderColor: "#a855f7", textAlign: "center", animation: "fadeIn 0.5s ease" }}>
              ...
              <Typography variant="body2" sx={{ color: "text.secondary" }}>
                Final Result: {dp[n] !== null && dp[n] !== undefined ? dp[n] : tree[`fib(${n})-1`]?.result}
              </Typography>
            </Box>
          )}
        </Box>
      ...
      <Box sx={{ flex: 1, display: "flex", justifyContent: "center" }}>
        {mode === "tabulation" ? renderTabulation() : (rootId ? (
          <Box id="fib-tree-container" sx={{ width: "fit-content", minWidth: 800, position: "relative" }}>
